...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	if relayInfo != nil {
		...
		if relayInfo.IsFallbackRouting() {
			other.SetAdmin("fallback_model", relayInfo.FallbackModelName)
			other.SetAdmin("fallback_attempt_index", relayInfo.FallbackAttemptIndex)
			other.SetAdmin("fallback_attempt_model", relayInfo.FallbackAttemptModelName)
		}
